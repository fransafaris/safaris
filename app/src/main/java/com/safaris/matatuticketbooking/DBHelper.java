...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String DBNAME = "MatatuTicketBooking.db";

    public <Context> DBHelper(Context context) {
        super((android.content.Context) context, "MatatuTicketBooking.db", null, 1);
    }
    ...
    public void onUpgrade(SQLiteDatabase MyDB, int oldVersion, int newVersion) {
        ...
        MyDB.execSQL("drop Table if exists matatus");
    ...
    public Boolean insertBus(String id, String departure, String arrival, String date, String total_seats)
    {
        ...
        long result = MyDB.insert("matatus",null,contentValues);
        if(result==-1) return false;
        else
            return true;
    }

    public Boolean updateMatatu(String id, String departure, String arrival, String date, String total_seats)
    {
        SQLiteDatabase MyDB = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("departure", departure);
        ...
        contentValues.put("date", date);
        contentValues.put("total_seats", total_seats);
        Cursor cursor = MyDB.rawQuery("Select * from matatus where id = ?", new String[] {id});
        if (cursor.getCount()>0){
            long result = MyDB.update("matatus", contentValues,"id=?", new String[] {id});
            if(result==-1) return false;
            else
                return true;
        }
    ...
    public Boolean deleteBus(String id)
    {
        ...
        if (cursor.getCount()>0){
            long result = MyDB.delete("matatus","id=?", new String[] {id});
            if(result==-1) return false;
            else
                return true;
        }
    ...
    public Cursor viewmatatus()
    {
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("Select * from matatus", null);
        return cursor;
    }
